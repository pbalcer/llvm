...
function generateExtraInfo(data) {
    ...
    return labels.map(label => {
        ...
        if (latestRun) {
            return `<div class="extra-info-entry"><strong>${label}:</strong> ${formatCommand(latestRun.result)}<br><em>Description:</em> ${latestRun.result.description}</div>`;
        }
        return `<div class="extra-info-entry"><strong>${label}:</strong> No data available</div>`;
    }).join('');
}
...
function formatCommand(run) {
    ...
    // so that commands look normal: ./app --params
    if (command.length > 0) {
        command[0] = `./${command[0]}`;
    }
...
function downloadChart(canvasId, label) {
    ...
    if (chart) {
        // we do the temp canvas to make sure the chart is on a white background
        ...
        // it does add a bit of clutter to the chart
        chart.data.datasets.forEach((dataset, datasetIndex) => {
            ...
            meta.data.forEach((point, index) => {
                ...
                ctx.font = '10px Verdana'; // I find it slightly more readable
                ...
                ctx.fillText(value, position.x - 20, position.y - 5);
            });
        });
