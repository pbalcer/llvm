...
struct urBatchedQueueTest : uur::urContextTest {
  ...
  void vectorOfSubmittedBatchesIsClearedHelper() {
    ...
    // Initially, the vector of batches submitted for execution is empty. After every iteration, each queueFlush results in submitting the current batch for execution, then pushing the submitted batch to the vector of submitted batches. Therefore, after initialSlotsForBatches we reach the maximum arbitrary chosen capacity (initialSlotsForBatches)
    ...
    // The maximum arbitrary set capacity is reached, but the vector is not cleared
    ...
    // When trying to push another submitted batch to the vector of submitted batches after reaching its arbitrary set capacity, the vector is cleared as a result of queueFinish. Submitted batches (regular command lists) are returned to the command list cache in their destructors.
...
void isEventCompleted(ur_event_handle_t &event) {
  ASSERT_EQ(zeEventHostSynchronize(event->getZeEvent(), UINT64_MAX),
            ZE_RESULT_SUCCESS);
}
/*
...
TEST_P(urBatchedQueueTest, RunBatchIfNeededCommandBuffer) {
  ...
  // ASSERT_NO_FATAL_FAILURE(isEventCompleted(eventAfterEnqueueCmdBuff));
